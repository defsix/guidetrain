...
- **`profiles.equipment` needs `migrations/0003_equipment.sql` applied, and
  this one is not optional to defer.** `profileToRow` sends an `equipment` key
  on every upsert now, so until the column exists, syncing a profile fails
  outright for a signed-in user — caught by the existing error handling
  rather than a crash, but sync silently stops working until the migration
  runs. `text[]` rather than a boolean per equipment type, since the set of
